...
machineSensorDataSchema.post('validate', function (doc) {
    logger.info(`Machine sensor with id: ${doc._id}, Serial: ${doc.serial} passed validation.`);
});

// Post-update hook
machineSensorDataSchema.post('findOneAndUpdate', function (doc) {
    if (doc) {
        logger.info(`Machine sensor with id: ${doc._id}, Serial: ${doc.serial} was updated.`);
    } else {
...
machineSensorDataSchema.post('findOneAndDelete', function (doc) {
    if (doc) {
        logger.info(`Machine sensor with id: ${doc._id}, Serial: ${doc.serial} was deleted.`);
    } else {
