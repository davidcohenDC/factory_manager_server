...
const createMachine = async (machineData) => {
    try {
        ...
        logger.info(`Machine created with id: ${machine._id}`, { machineId: machine._id, serial: machine.serial });
        ...
        if (error.message === 'Machine with this serial already exists.') {
            ...
            return { success: false, error: 'Machine with this serial already exists.' };
        } else {
...
const getMachineById = async (id) => {
    try {
        ...
        if (!machine) return { success: false, error: 'Machine not found', status: 404 };
        logger.info(`Machine retrieved with id: ${id}`);
        return { success: true, data: machine };
    } catch (error) {
        logger.error(`Error retrieving machine by id: ${error.message}`, { stack: error.stack });
        return { success: false, error: 'Failed to retrieve machine by id', status: 500 };
    }
}
...
const getMachineBySerial = async (serial) => {
    try {
        ...
        logger.error(`Error retrieving machine by serial: ${error.message}`, { stack: error.stack });
        return { success: false, error: 'Failed to retrieve machine by serial', status: 500 };
    }
}
...
const updateMachineById = async (id, updateData) => {
    try {
        ...
        if (!updatedMachine) return { success: false, error: 'Machine not found', status: 404 };
...
const updateMachineBySerial = async (serial, updateData) => {
    try {
        const updatedMachine = await machineRepository.updateMachineBySerial(serial, updateData);
        if (!updatedMachine) return { success: false, error: 'Machine not found', status: 404 };
...
const deleteMachineById = async (id) => {
    try {
        ...
        if (!deletedMachine) return { success: false, error: 'Machine not found', status: 404 };
...
const getAllMachines = async (limit, offset) => {
    try {
        ...
        logger.error(`Error retrieving all machines: ${error.message}`, { stack: error.stack });
        return { success: false, error: 'Failed to retrieve all machines', status: 500 };
    }
}
