...
const createUser = async (userData) => {
    try {
        ...
        logger.info(`User created with id: ${user._id}`, { userId: user._id, email: user.email });
...
const checkUserEmail = async (email) => {
    try {
        ...
        logger.error(`Error checking user email: ${error.message}`, { stack: error.stack });
...
const getUserByEmail = async (email) => {
    try {
        ...
        logger.error(`Error retrieving user by email: ${error.message}`, { stack: error.stack });
        return { success: false, error: 'Failed to retrieve user by email', status: 500 };
    }
};
...
const getUserById = async (id) => {
    try {
        ...
        logger.error(`Error retrieving user by id: ${error.message}`, { stack: error.stack });
        return { success: false, error: 'Failed to retrieve user by id', status: 500 };
    }
};
...
const updateUserById = async (id, updateData) => {
    try {
        ...
        if (!updatedUser) return { success: false, error: 'User not found', status: 404 };
...
const deleteUserById = async (id) => {
    try {
        ...
        if (!deletedUser) return { success: false, error: 'User not found', status: 404 };
        logger.info(`User deleted with id: ${id}`);
        return { success: true, data: deletedUser };
    } catch (error) {
        logger.error(`Error deleting user by id: ${error.message}`, { stack: error.stack });
